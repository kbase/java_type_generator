...
public class KbModule {
	...
	public void loadFromList(List<?> data, JSyncProcessor subst) {
		...
		for (Map<?,?> compProps : Utils.getListOfMapProp(props, "module_components", subst)) {
			String compType = Utils.getPerlSimpleType(compProps);
			if (compType.equals("Typedef")) {
				moduleComponents.add(new KbTypedef().loadFromMap(compProps, subst));
			} else if (compType.equals("Funcdef")) {
				moduleComponents.add(new KbFuncdef().loadFromMap(compProps, subst));
			} else {
		...
		for (Map<?,?> infoProps : Utils.repareTypingMap((List<?>)data.get(1), subst)) {
			typeInfoList.add(new KbTypeInfo().loadFromMap(infoProps, subst));
		}
		...
		for (Object key : typeMap.keySet()) {
			...
			Map<?,?> typeProps = Utils.propMap(typeMap, typeName, subst);
			nameToType.put(typeName, Utils.createTypeFromMap(typeProps, subst));
		}
